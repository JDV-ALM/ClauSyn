...
class AgedReceivableCustomHandler(models.AbstractModel):
    ...
    def _aged_partner_report_custom_engine_common(self, options, internal_type, current_groupby, next_groupby, offset=0, limit=None):
        ...
        def build_result_dict(report, query_res_lines):
            ...
            rslt.update({'sales_person': None})

            for query_res in query_res_lines:
                sales_person_user = self.env['res.users'].browse(query_res['sales_person'][0]) if len(
                    query_res['sales_person']) == 1 else None
                if sales_person_user:
                    rslt['sales_person'] = sales_person_user.name
                else:
                    rslt['sales_person'] = ""
        ...
        query = SQL(
            ...
            SELECT
                ...
                ARRAY_AGG(DISTINCT users.id) AS sales_person,
            ...
            LEFT JOIN res_users users ON users.id = partner.user_id
    ...
    #     return {
    #         'templates': {
    #             'AccountReportFilters': 'wm_ar_salesperson_filter.SalespersonFilters',
    #         },
    #     }

    # To solve issue in when check options -> Unfold all
